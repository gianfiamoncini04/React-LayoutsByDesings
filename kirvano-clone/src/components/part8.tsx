			...
			<section className="flex flex-col gap-12 px-80 py-28">
				<div className="flex flex-col gap-2 justify-center items-center text-center break-normal">
					...
					<p className="text-lg w-full max-w-[600px]">
						Ofereça uma experiência verdadeiramente exclusiva e atraente para os
						seus membros através das nossas ferramentas de personalização.
					</p>
				</div>
				<div className="grid grid-cols-2 gap-6  text-lg">
					<div className="flex flex-row bg-[#171717] text-white rounded-xl p-8 gap-4">
						<span className="flex flex-col justify-center items-start gap-4">
							...
							<p className="font-normal">
								Receba pagamentos de forma simples, com uma plataforma sem
								custos ocultos.
							</p>
							<Button
								class="w-full items-center justify-center !p-4"
								valor="Garantir taxa"
							/>
						</span>
					...
					<div className="flex flex-col border border-zinc-300 rounded-xl p-8 gap-4">
						...
						<p>
							...
							de 100k por mês, podemos ajudá-lo com Condições especiais.
						</p>
						<Button
							class="bg-[#e6e7ea] !text-black text-base justify-center inline-flex gap-2 py-4"
							iconleft={<FiPhone />}
							valor="Falar com um especialista"
						/>
					</div>
				</div>
				<div className="grid grid-cols-3 gap-4">
					{dataSix.map(([img, addInfo, style, title, topics], index) => (
						<div
							key={index}
							className="border border-zinc-300 rounded-lg p-12 gap-4 flex flex-col justify-center items-center">
							...
							<h1 className="font-black text-base">{title}</h1>
							<div className="flex flex-col items-start justify-center gap-2">
								{Array.from(topics).map((tpc, index) => (
									<p
										key={index}
										className="inline-flex justify-center items-start gap-2 text-base">
